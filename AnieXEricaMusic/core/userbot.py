...
class Userbot(Client):
    ...
    async def start(self):
        ...
        if config.STRING1:
            ...
            try:
                ...
                LOGGER(__name__).warning(f"Failed to join some chats for Assistant 1: {str(e)}")
            assistants.append(1)
            try:
                await self.one.send_message(config.LOG_GROUP_ID, "Assistant Started 1")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant Account 1 has failed to access the log Group. Error: {str(e)}")
            ...
            try:
                ...
                LOGGER(__name__).warning(f"Error while sending messages for Assistant 1: {str(e)}")
        ...
        if config.STRING2:
            ...
            try:
                ...
                LOGGER(__name__).warning(f"Failed to join some chats for Assistant 2: {str(e)}")
            assistants.append(2)
            try:
                await self.two.send_message(config.LOG_GROUP_ID, "Assistant Started 2")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant Account 2 has failed to access the log Group. Error: {str(e)}")
        ...
        if config.STRING3:
            ...
            try:
                ...
                LOGGER(__name__).warning(f"Failed to join some chats for Assistant 3: {str(e)}")
            assistants.append(3)
            try:
                await self.three.send_message(config.LOG_GROUP_ID, "Assistant Started 3")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant Account 3 has failed to access the log Group. Error: {str(e)}")
        ...
        if config.STRING4:
            ...
            try:
                ...
                LOGGER(__name__).warning(f"Failed to join some chats for Assistant 4: {str(e)}")
            assistants.append(4)
            try:
                await self.four.send_message(config.LOG_GROUP_ID, "Assistant Started 4")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant Account 4 has failed to access the log Group. Error: {str(e)}")
        ...
        if config.STRING5:
            ...
            try:
                ...
                LOGGER(__name__).warning(f"Failed to join some chats for Assistant 5: {str(e)}")
            assistants.append(5)
            try:
                await self.five.send_message(config.LOG_GROUP_ID, "Assistant Started 5")
            except Exception as e:
                LOGGER(__name__).warning(f"Assistant Account 5 has failed to access the log Group. Error: {str(e)}")
    ...
    async def stop(self):
        ...
        try:
            ...
            LOGGER(__name__).warning(f"Error while stopping assistants: {str(e)}")
