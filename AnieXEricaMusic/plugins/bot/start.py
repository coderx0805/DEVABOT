...
async def start_pm(client, message: Message, _):
    if len(message.text.split()) == 1:
        ...
        await loading_1.edit_text("ʟᴏᴀᴅɪɴɢ..")
        await asyncio.sleep(0.1)
        await loading_1.edit_text("ᴀʟᴍᴏsᴛ ʜᴇʀᴇ...")
        ...
        for member in message.new_chat_members:
            ...
            if isinstance(config.OWNER_ID, int): 
                if member.id == config.OWNER_ID:
                    owner = f"ᴍʏ ʟᴏʀᴅ ɪs ɪɴ ᴛʜᴇ ᴄʜᴀᴛs ɴᴏᴡ 🔱"
            ...
            if isinstance(SUDOERS, int): 
                if member.id == SUDOERS:
                    AMBOT = f"ᴅɪsᴀsᴛᴇʀ ʟᴇᴠᴇʟ ᴜsᴇʀ ʜᴇʀᴇ ❄"
                ...
                if member.id in SUDOERS:
                    AMBOT = f"ᴡᴏʀʟᴅ ᴀᴅᴍɪɴ ɪs ɴᴏᴡ ʜᴇʀᴇ"
                    sent_message = await message.reply_text(AMBOT, reply_markup=buttons)
                    await asyncio.sleep(60)
                    await sent_message.delete()  
                    return
